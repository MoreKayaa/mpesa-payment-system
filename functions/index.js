...
// Environment config
const consumerKey = functions.config().mpesa.consumer_key || process.env.MPESA_CONSUMER_KEY;
const consumerSecret = functions.config().mpesa.consumer_secret || process.env.MPESA_CONSUMER_SECRET;
const passkey = functions.config().mpesa.passkey || process.env.MPESA_PASSKEY;
const shortcode = functions.config().mpesa.shortcode || process.env.MPESA_SHORTCODE;
...
exports.getAccessToken = functions.https.onCall(async (data, context) => {
  try {
    ...
    throw new functions.https.HttpsError('internal', 'Failed to get access token');
  }
});
...
exports.initiateSTKPush = functions.https.onCall(async (data, context) => {
  try {
    ...
    const stkRequestBody = {
      ...
      Password: password,
      Timestamp: timestamp,
      TransactionType: business.mpesaConfig?.transactionType || "CustomerPayBillOnline",
      Amount: data.amount,
      PartyA: phoneNumber,
      PartyB: businessShortcode,
      PhoneNumber: phoneNumber,
      CallBackURL: business.mpesaConfig?.callbackUrl || 
    ...
    if (resultCode === 0) {
      ...
      webhooksSnapshot.forEach(doc => {
        ...
        webhookPromises.push(
          axios.post(webhook.url, webhookPayload, {
